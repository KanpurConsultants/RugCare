    ...
    public class WeavingInvoiceController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult PostSummary(JobInvoiceSummaryDetailViewModel vm)
        {
            ...
            if (AdvanceAdjustedDataTable.Rows.Count > 0)
            {
                ...
                {
                    sqlConnection.Open();
                    using (SqlCommand cmd = new SqlCommand("" + ConfigurationManager.AppSettings["DataBaseSchema"] + ".spGetLedgerAdj_Advance"))
                    {
            ...
            JobInvoiceHeader JobInvoiceHeader = new JobInvoiceHeaderService (_unitOfWork).Find(vm.JobInvoiceHeaderId);
            JobInvoiceHeader.IsLedgerAdjusted = true;
            JobInvoiceHeader.ModifiedDate = DateTime.Now;
            JobInvoiceHeader.ModifiedBy = User.Identity.Name;
            JobInvoiceHeader.ObjectState = Model.ObjectState.Modified;
            new JobInvoiceHeaderService(_unitOfWork).Update(JobInvoiceHeader);
