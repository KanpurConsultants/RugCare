...
namespace Jobs.Areas.Rug.Controllers
{
    ...
    public class JobReceiveQAPenaltyController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult _CreateMultple(int Id) //Id ==> JobReceiveQALineId
        {
            JobReceiveQALine L = new JobReceiveQALineService(db, _unitOfWork).Find(Id);
            JobReceiveQAHeader H = new JobReceiveQAHeaderService(db).Find(L.JobReceiveQAHeaderId);
            DocumentType D = new DocumentTypeService(_unitOfWork).Find(H.DocTypeId);
            ...
            List<JobReceiveQAPenaltyViewModel> s = new List<JobReceiveQAPenaltyViewModel>();

            for (int i = 0; i < 5; i++)
            {
                JobReceiveQAPenaltyViewModel temp = new JobReceiveQAPenaltyViewModel();
                temp.DocTypeId = H.DocTypeId;
                temp.JobReceiveQALineId = Id;
                s.Add(temp);
            }


            JobReceiveQAPenaltyListViewModel svm = new JobReceiveQAPenaltyListViewModel();
            svm.JobReceiveQAPenaltyViewModel = s;
            svm.DocTypeId = D.DocumentTypeId;
            PrepareViewBag();
            if (!string.IsNullOrEmpty((string)TempData["CSEXCL"]))
            {
                ViewBag.CSEXCL = TempData["CSEXCL"];
            ...
            ViewBag.LineMode = "Create";
            ViewBag.DocNo = D.DocumentTypeName + "-" + H.DocNo;
            return PartialView("_CreateMultple", svm);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult _CreateMultplePost(JobReceiveQAPenaltyListViewModel svm)
        {
            JobReceiveQALine L = new JobReceiveQALineService(db, _unitOfWork).Find(svm.JobReceiveQAPenaltyViewModel[0].JobReceiveQALineId);            
            JobReceiveQAHeader temp = new JobReceiveQAHeaderService(db).Find(L.JobReceiveQAHeaderId);
            ...
            try
            {
                ...
                BeforeSave = JobReceiveQADocEvents.beforeLineSaveEvent(this, new JobEventArgs(svm.JobReceiveQAPenaltyViewModel[0].JobReceiveQAPenaltyId, EventModeConstants.Add), ref db);
            ...
            List<JobReceiveQAPenaltyViewModel> PenaltList = svm.JobReceiveQAPenaltyViewModel;
            ...
            if (ModelState.IsValid && BeforeSave && !EventException)
            {
                foreach (var Item in PenaltList)
                {
                    if (Item.ReasonId != 0)
                    {
                        JobReceiveQAPenalty s = Mapper.Map<JobReceiveQAPenaltyViewModel, JobReceiveQAPenalty>(Item);
                        _JobReceiveQAPenaltyService.Create(s, User.Identity.Name);
                ...
                //return RedirectToAction("_Create", new { id = svm.JobReceiveQAPenaltyViewModel[0].JobReceiveQALineId });
                return Json(new { success = true });
            }
        ...
        public JsonResult GetReasons(string searchTerm, int pageSize, int pageNum, int filter)//filter:PersonId
        {
            DocumentType DT = new DocumentTypeService(_unitOfWork).Find(filter);

            var temp = new ReasonService(_unitOfWork).GetReasons(DT.DocumentCategoryId, searchTerm).Skip(pageSize * (pageNum - 1)).Take(pageSize).ToList();

            var count = new ReasonService(_unitOfWork).GetReasons(DT.DocumentCategoryId, searchTerm).Count();
        ...
        public ActionResult DeletePost(JobReceiveQAPenaltyViewModel vm)
        {
            ...
            if (BeforeSave && !EventException)
            {
                ...
                List<JobReceiveQAPenalty> PenaltyLinesList = (from Pl in db.JobReceiveQAPenalty where Pl.JobReceiveQALineId == L.JobReceiveQALineId && Pl.JobReceiveQAPenaltyId != vm.JobReceiveQAPenaltyId select Pl).ToList();
                Decimal TotalPenalty = 0;
                if (PenaltyLinesList.Count() != 0)
                {
                    TotalPenalty = PenaltyLinesList.Sum(i => i.Amount) ;
                }
                else
                {
                    TotalPenalty = 0;
