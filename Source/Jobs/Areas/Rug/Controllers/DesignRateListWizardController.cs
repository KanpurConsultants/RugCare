...
{
    [Authorize]
    public class DesignRateListWizardController : System.Web.Mvc.Controller
    {
        ...
        public DesignRateListWizardController(IUnitOfWork unitOfWork, IExceptionHandlingService exec)
        {
            _exception = exec;
        ...
        public ActionResult DesignRateList(int Id)
        {
            ...
            ViewBag.Name = "Design Rate List -" + db.RateListHeader.Find(Id).RateListName;
        ...
        public ActionResult UpdateDesignRate(int ProductGroupId, decimal? Rate, decimal? Incentive, decimal? Discount, decimal? Loss, int RateListHeaderId)
        {
            XElement Modifications;
            bool Flag = new RateListLineService(_unitOfWork).UpdateRateListLineForDesign(ProductGroupId, Rate ?? 0, Incentive ?? 0, Discount ?? 0, Loss ?? 0, RateListHeaderId, User.Identity.Name, out Modifications);
