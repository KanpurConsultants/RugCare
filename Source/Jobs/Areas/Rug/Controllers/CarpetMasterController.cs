...
namespace Jobs.Areas.Rug.Controllers
{
    ...
    public class CarpetMasterController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult CopyFromExisting(CopyFromExistingDesignViewModel vm)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                foreach (FinishedProduct item in list)
                {
                    ...
                    fp.IsConsumptionPcsWise = item.IsConsumptionPcsWise;
        ...
        public ActionResult Edit(int id, bool sample)//ProductGroupId
        {
            ...
            if (product != null)
            {
                ...
                CarpetMasterViewModel vm = new CarpetMasterViewModel()
                {
                    ...
                    IsConsumptionPcsWise= (bool) productInfo.IsConsumptionPcsWise,
                ...
                else
                {
                    ...
                    if (temp.Count() > 0)
                    {
                        ...
                        foreach (var item in temp)
                        {
                            ...
                            prod.IsConsumptionPcsWise = vm.IsConsumptionPcsWise;
        ...
        public ActionResult EditSize(int id)//ProductId
        {
            ...
            var JOL = new JobOrderLineService(_unitOfWork).GetJobOrderLine_ByProductId(id);

            if (JOL != null)
                vm.IsOrderCreated = true;
            else
                vm.IsOrderCreated = false;
        ...
        public ActionResult EditSizePost(CarpetMasterViewModel vm, FormCollection collection, bool? IsMultiSave = false)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (vm.ProductId == 0)
                {
                    ...
                    pro.IsConsumptionPcsWise = vm.IsConsumptionPcsWise;
                    pro.Tags = vm.Tags;
                    pro.CBM = vm.CBM;
                    ...
                    var ProductGroup = db.ProductGroups.Find(vm.ProductGroupId);
                    var Colour = db.Colour.Find(vm.ColourId);

                    var BOMItem = ProductGroup.ProductGroupName + "-" + Colour.ColourName + "-BOM";

                    var BomProduct = (from P in db.Product 
                                      where P.ProductName == BOMItem
                                      select new
                                      {
        ...
        public ActionResult DeleteConfirmedLine(CarpetMasterViewModel vm)
        {
            ...
            var RateListLine = (from p in db.RateListLine
                                  where p.ProductId == productId
                                  select p).ToList();

            foreach (var item5 in RateListLine)
            {
                new RateListLineService(_unitOfWork).Delete(item5.RateListLineId);
            }
        ...
        public ActionResult DeleteConfirmed(ReasonViewModel vm)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                foreach (var item in productslist)
                {
                    ...
                    foreach (var item5 in RateListLine)
                    {
                        new RateListLineService(_unitOfWork).Delete(item5);
                    }
