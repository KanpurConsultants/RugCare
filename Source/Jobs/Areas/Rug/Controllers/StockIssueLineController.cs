...
namespace Jobs.Areas.Rug.Controllers
{
    ...
    public class StockIssueLineController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult _ForParentsCostcenter(int id, int sid)
        {
            RequisitionFiltersForIssue vm = new RequisitionFiltersForIssue();
            StockHeader Header = new StockHeaderService(_unitOfWork).Find(id);
            vm.DocumentTypeSettings = new DocumentTypeSettingsService(_unitOfWork).GetDocumentTypeSettingsForDocument(Header.DocTypeId);
            vm.StockHeaderId = id;
            vm.PersonId = sid;
            return PartialView("_FiltersForParentsCostcenter", vm);
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult _FilterPostParentCostcenter(RequisitionFiltersForIssue vm, string All)
        {
            List<StockLineViewModel> temp = _StockLineService.GetIssuedForFiltersByParentCostCenter(vm, (string.IsNullOrEmpty(All) ? false : true)).ToList();
