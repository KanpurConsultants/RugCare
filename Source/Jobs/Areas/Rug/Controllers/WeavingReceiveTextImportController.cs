...
{
    [Authorize]
    public class WeavingReceiveTextImportController : System.Web.Mvc.Controller
    {
        private ApplicationDbContext db = new ApplicationDbContext();

        IUnitOfWork _unitOfWork;

        public WeavingReceiveTextImportController(IUnitOfWork unitOfWork)
        {
        ...
        public ActionResult WeavingReceiveTextImport(int id, int? GodownId)
        {
            ...
            do
            {
                ...
                if (Line != null)
                {
                    ...
                    if (PrevSupplierCode != dr["ProductUidName"].ToString())
                    {
                        if (PrevDoNo == "")
                        {
                        ...
                        PrevSupplierCode = dr["ProductUidName"].ToString();
            ...
            {
                sqlConnection.Open();
                using (SqlCommand cmd = new SqlCommand("" + ConfigurationManager.AppSettings["DataBaseSchema"] + ".ProcImportWeavingReceiveFromTextFile"))
                {
        ...
        public ActionResult ReturnToRoute(int id)//Document Type Id
        {
            ...
            return Redirect(System.Configuration.ConfigurationManager.AppSettings["JobsDomain"] + "/" + "Rug/WeavingReceiveQACombined" + "/" + "Index" + "/" + id);
        }
    }
