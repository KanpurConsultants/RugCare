...
namespace Jobs.Areas.Rug.Controllers
{
    ...
    public class ProductConsumptionHeaderController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult GetFinishedMaterial(string searchTerm, int pageSize, int pageNum, int? filter)
        {
            var Query = _BomDetailService.GetBaseProduct(filter, searchTerm);
        ...
        {
            var DocType = new DocumentTypeService(_unitOfWork).FindByName(MasterDocTypeConstants.ProductConsumption);
            int DocTypeId = 0;

            if (DocType != null)
                DocTypeId = DocType.DocumentTypeId;
            else
                return View("~/Views/Shared/InValidSettings.cshtml").Warning("Document Type named " + MasterDocTypeConstants.ProductConsumption + " is not defined in database.");
        ...
        public ActionResult Create(ProductConsumptionHeaderViewModel svm)
        {
            ...
            if (ModelState.IsValid)
            {
                if (svm.BaseProductId == 0)
                {
                    ...
                    if (SProd.ProductName.Length > 46)
                    {
                        product.ProductCode = SProd.ProductName.ToString().Substring(0, 46) + "-Bom";
                    }
                    else
                    {
        ...
        public ActionResult DeleteConfirmed(ReasonViewModel vm)
        {
            if (ModelState.IsValid)
            {
                ...
                ActivityLog al = new ActivityLog()
                {
                    ...
                    DocTypeId = new DocumentTypeService(_unitOfWork).FindByName(TransactionDoctypeConstants.BOM).DocumentTypeId,
                ...
                var temp = (from L in db.BomDetail
                            ...
                            select new { BomDetailId = L.BomDetailId, LastProductId=L.LastProductId }).ToList();


                foreach (var item in temp)
                {
                    if (item.LastProductId !=null)
                    {
                        BomDetail Bd = db.BomDetail.Find(item.BomDetailId);
                        Bd.ProductId = (int) Bd.LastProductId;
                        Bd.LastProductId = null;
                        Bd.ModifiedDate = DateTime.Now;
                        Bd.ModifiedBy = User.Identity.Name;
                        Bd.ObjectState = Model.ObjectState.Modified;
                        _BomDetailService.Update(Bd);
                    }
                     else
                    _BomDetailService.Delete(item.BomDetailId);
                }
        ...
        public JsonResult GetProductDetailJson(int ProductId)
        {
            ...
            return Json(new { ProductName = Prod.ProductName, ProductQualityName = productgroupquality.ProductQualityName, Weight = Prod.StandardWeight });
        }
        ...
        public ActionResult CopyFromExisting(CopyFromExistingProductConsumptionViewModel vm)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                Product Fromprod = new ProductService(_unitOfWork).Find(vm.FromProductId);

                if (prod.ProductName.Length > 46)
                {
                    NewProduct.ProductCode = prod.ProductName.ToString().Substring(0, 46) + "-Bom";
                }
                else
                {
                ...
                NewProduct.StandardWeight = Fromprod.StandardWeight;
                ...
                if (ExistingRec != null)
                {
                    ExistingRec.LastProductId = vm.FromProductId;
                ...
                foreach (BomDetail item in bomdetList)
                {
                    ...
                    bomdet.BaseProcessId = item.BaseProcessId;
                    bomdet.MainWeight = item.MainWeight;
