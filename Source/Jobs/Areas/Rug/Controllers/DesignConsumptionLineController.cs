        ...
        private void PrepareViewBag(DesignConsumptionLineViewModel svm)
        {
            ...
            var TotalMainContents = (from L in db.BomDetail
                             ...
                             select new
                             {
                                 ...
                                 TotalPer = Result.Sum(i => i.L.ConsumptionPer)
                             }).FirstOrDefault();
            ...
            if (TotalMainContents != null  && svm.Weight!=0)
            {
                ...
                Decimal TotalMainContentPercentage = Math.Round(TotalMainContents.TotalPer, 2);
            ...
            {
                var MainContents = (from L in db.BomDetail
                                    join P in db.Product on L.ProductId equals P.ProductId into ProductTable
                                    from ProductTab in ProductTable.DefaultIfEmpty()
                                    join pcon in ProductFaceContentGroups on ProductTab.ProductGroupId equals pcon.ProductGroupId into ProductFaceContentTable
                                    from ProductFaceContentTab in ProductFaceContentTable.DefaultIfEmpty()
                                    where L.BomDetailId == svm.BomDetailId && ((int?)ProductFaceContentTab.ProductGroupId ?? 0) != 0
                                    group new { L } by new { L.BaseProductId } into Result
                                    select new
                                    {
                                        TotalQty = Result.Sum(i => i.L.Qty),
            ...
            CarpetSkuSettings CS = new CarpetSkuSettingsService(_unitOfWork).GetCarpetSkuSettings(DivisionId, SiteId);
            svm.isVisibleMainWeightinBOM = CS.isVisibleMainWeightinBOM;
        }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (svm.BaseProductId == 0)
                {
                    ...
                    if (ProductGroup != null && Colour != null)
                    {
                        ...
                        {
                            BomDetail BD = new BomDetailService(_unitOfWork).GetBomDetailList(item.ProductId).FirstOrDefault();
                            ...
                            if (BD == null)
                            {
                                ...
                                Product BProduct = new ProductService(_unitOfWork).Find(item.ProductId);
                                BProduct.GrossWeight = svm.Weight;
                                BProduct.ModifiedDate = DateTime.Now;
                                BProduct.ModifiedBy = User.Identity.Name;
                                BProduct.ObjectState = Model.ObjectState.Modified;
                                new ProductService(_unitOfWork).Update(BProduct);
                            }
                        }
                    }
                ...
                if (svm.BomDetailId == 0)
                {
                    ...
                    bomdetail.MainWeight = svm.Weight;
