...
{
    [Authorize]
    public class JobRatePenaltyUpdateController : System.Web.Mvc.Controller
    {
        ...
        public JobRatePenaltyUpdateController(IUnitOfWork unitOfWork, IExceptionHandlingService exec)
        {
            _exception = exec;
        ...
        public ActionResult Index(int Id)//DocumentTypeId
        {
            ...
            ViewBag.Title = "Penalty Update : " + new DocumentTypeService(_unitOfWork).Find(Id).DocumentTypeName;
            return View("JobRatePenaltyUpdate");
        }

        public JsonResult JobReceiveList(JobRatePenaltyUpdateFilterArgs Fvm)//DocumentTypeId
        {
            int DivisionId = (int)System.Web.HttpContext.Current.Session["DivisionId"];
            int SiteId = (int)System.Web.HttpContext.Current.Session["SiteId"];

            int DocumentTypeId = Fvm.DocumentTypeId;
            ...
            //            join JW in db.Persons on H.JobWorkerId equals JW.PersonID into tableJW
            //            from tabJW in tableJW.DefaultIfEmpty()
            //            join L in db.JobReceiveLine on H.JobReceiveHeaderId equals L.JobReceiveHeaderId into tableL
            //            from tabL in tableL.DefaultIfEmpty()
            //            join JOL in db.JobOrderLine on tabL.JobOrderLineId equals JOL.JobOrderLineId into tableJOL
            ...
            //            join ODT in db.DocumentType on tabJOH.DocTypeId equals ODT.DocumentTypeId into tableODT
            //            from tabODT in tableODT.DefaultIfEmpty()
            //            join JIL in db.JobInvoiceLine on tabL.JobReceiveLineId equals JIL.JobReceiveLineId into tableJIL
            //            from tabJIL in tableJIL.DefaultIfEmpty()
            //            join P in db.Product on tabJOL.ProductId equals P.ProductId into tableP
            //            from tabP in tableP.DefaultIfEmpty()
            //            join PU in db.ProductUid on tabL.ProductUidId equals PU.ProductUIDId into tablePU
            //            from tabPU in tablePU.DefaultIfEmpty()
            //            join PU1 in db.ProductUid on tabL.LotNo equals PU1.LotNo into tablePU1
            //            from tabPU1 in tablePU1.DefaultIfEmpty()
            //            join JRQ in db.JobReceiveQALine on tabL.JobReceiveLineId equals JRQ.JobReceiveLineId into tableJRQ
            //            from tabJRQ in tableJRQ.DefaultIfEmpty()
            //            join JRQP in db.JobReceiveQAPenalty on tabJRQ.JobReceiveQALineId equals JRQP.JobReceiveQALineId into tableJRQP
            ...
            //                && ((bool?)tabJW.IsSisterConcern ?? false) == false && H.CreatedBy != "System" && H.Status == (int)StatusConstants.Submitted
            //            orderby H.DocDate, H.DocNo
            //            group new { H, tabL, tabJRQP, tabPU, tabPU1, tabODT, tabJOH, tabJW, tabR, tabP, tabJOL } by new { H.JobReceiveHeaderId } into Result
            //            select new 
            //            {
            //                JobReceiveLineId = Result.Max(i => i.tabL.JobReceiveLineId),
            //                JobOrderLineId = Result.Max(i => i.tabL.JobOrderLineId),
            //                JobReceiveQAPenaltyId =  Result.Max(i => (int?) i.tabJRQP.JobReceiveQAPenaltyId),
            //                ProductUidName = Result.Max(i => i.tabPU.ProductUidName) == null ? Result.Max(i => i.tabPU1.ProductUidName) : Result.Max(i => i.tabPU.ProductUidName),
            //                DocNo = Result.Max(i => i.H.DocNo),
            //                OrderNo = Result.Max(i => i.tabODT.DocumentTypeShortName) +"-"+ Result.Max(i => i.tabJOH.DocNo),
            //                DocDate = Result.Max(i => i.H.DocDate),
            //                JobWorkerName = Result.Max(i => i.tabJW.Name),
            //                PenaltyResion = Result.Max(i => i.tabR.ReasonName),
            //                PenaltyRemark = Result.Max(i => i.tabJRQP.Remark),
            //                ProductCategory = Result.Max(i => i.tabP.ProductCategory.ProductCategoryName),
            //                ProductName = Result.Max(i => i.tabP.ProductName),
            //                DealQty = Result.Max(i => (decimal?) i.tabL.DealQty),
            //                Rate = Result.Max(i => (decimal?) i.tabJOL.Rate),
            //                PenaltyAmount = Result.Max(i => (decimal?) i.tabJRQP.Amount)
            //            }).ToList();

            //select new JobRatePenaltyUpdateViewModel
            //            {
            //                JobReceiveLineId = tabL.JobReceiveLineId,
            //                JobOrderLineId = tabJOL.JobOrderLineId,
            //                JobReceiveQAPenaltyId = tabJRQP.JobReceiveQAPenaltyId,
            ...
            string mQry = @"SELECT L.JobReceiveLineId, Max(L.JobOrderLineId) AS JobOrderLineId, isnull(Max(JRQP.JobReceiveQAPenaltyId),0) AS JobReceiveQAPenaltyId,
                            ...
                            Max( CASE WHEN L.isHoldForInvoice = 1 THEN 'Hold For Invoice' + isnull(JRQP.Remark,'') ELSE isnull(JRQP.Remark,'') END )  PenaltyRemark,
                            Max(L.DealQty) AS DealQty, isnull(Max(JOL.Rate),0) AS Rate, isnull(sum(JRQP.Amount),0) AS PenaltyAmount
                            FROM
                             (SELECT * FROM web.ViewJobReceiveBalanceForInvoice V WITH(Nolock) WHERE V.DocTypeId = " + DocumentTypeId.ToString()  + @") V
                            LEFT JOIN web.JobReceiveHeaders H WITH(Nolock) ON H.JobReceiveHeaderId = V.JobReceiveHeaderId
                            ...
                            LEFT JOIN web.JobReceiveQAPenalties JRQP WITH(Nolock) ON JRQP.JobReceiveQALineId = JRQ.JobReceiveQALineId
                            LEFT JOIN web.Reasons R WITH(Nolock) ON R.ReasonId = JRQP.ReasonId
                            LEFT JOIN Web.ProductUids PU  WITH(Nolock) ON PU.ProductUIDId = L.ProductUidId OR PU.ProductUidName = L.LotNo
                            WHERE 1=1 AND isnull(JW.IsSisterConcern,0) =0 AND H.Status =1 " +
                            " AND H.SiteId = " + SiteId.ToString() + " " +
                            " AND H.DivisionId =  " + DivisionId.ToString() + " " +
                            " GROUP BY L.JobReceiveLineId order by Max(H.DocDate), Max(H.DocNo) ";

            List<JobReceivePendingToInvoice> list = db.Database.SqlQuery<JobReceivePendingToInvoice>(mQry).ToList();
            ...
            var temp = list.Select(m => new
            {
                ...
                JobReceiveQAPenaltyId = m.JobReceiveQAPenaltyId,
                ...
                ProductQuality = m.ProductQuality,
                ProductName = m.ProductName,
                DealQty = m.DealQty,
                Rate = m.Rate  == null ? 0 : m.Rate,
                PenaltyAmount = m.PenaltyAmount == null ? 0 : m.PenaltyAmount,
            }).ToList();

            return Json(new { data = temp }, JsonRequestBehavior.AllowGet);
        }

        public class JobReceivePendingToInvoice
        {
            ...
            public string IsHoldForInvoice { get; set; }
        }


        public ActionResult UpdateJobReceiveQAPenaltyAmount(int JobReceiveQAPenaltyId, decimal PenaltyAmount)
        {
            List<LogTypeViewModel> LogList = new List<LogTypeViewModel>();

            JobReceiveQAPenalty QAPenalty = new JobReceiveQAPenaltyService(db,_unitOfWork).Find(JobReceiveQAPenaltyId);
            JobReceiveQALine QL = new JobReceiveQALineService(db, _unitOfWork).Find(QAPenalty.JobReceiveQALineId);
            JobReceiveQAPenalty ExRec = new JobReceiveQAPenalty();
            ExRec = Mapper.Map<JobReceiveQAPenalty>(QAPenalty);

            QAPenalty.Amount = PenaltyAmount;
            new JobReceiveQAPenaltyService(db, _unitOfWork).Update(QAPenalty, User.Identity.Name);

            List<JobReceiveQAPenalty> PenaltyLinesList = (from Pl in db.JobReceiveQAPenalty where Pl.JobReceiveQALineId == QAPenalty.JobReceiveQALineId && Pl.JobReceiveQAPenaltyId != QAPenalty.JobReceiveQAPenaltyId select Pl).ToList();
            Decimal TotalPenalty = 0;
            if (PenaltyLinesList.Count() != 0)
            {
                TotalPenalty = PenaltyLinesList.Sum(i => i.Amount) + PenaltyAmount;
            }
            else
            {
            ...
            QL.PenaltyAmt = TotalPenalty;
            new JobReceiveQALineService(db, _unitOfWork).Update(QL, User.Identity.Name);

            _unitOfWork.Save();
            db.SaveChanges();


            JobReceiveQALine L = new JobReceiveQALineService(db, _unitOfWork).Find(QAPenalty.JobReceiveQALineId);
            ...
            LogActivity.LogActivityDetail(LogVm.Map(new ActiivtyLogViewModel
            {
                ...
                DocLineId = QAPenalty.JobReceiveQAPenaltyId,
        ...
        public ActionResult UpdateJobOrderLineRate(int JobOrderLineId, decimal Rate)
        {
            List<LogTypeViewModel> LogList = new List<LogTypeViewModel>();
            JobOrderLine JobOrderLine = new JobOrderLineService(_unitOfWork).Find(JobOrderLineId);
            JobOrderLine ExTempLine = new JobOrderLine();
            ExTempLine = Mapper.Map<JobOrderLine>(JobOrderLine);

            JobOrderHeader JobOrderHeader = new JobOrderHeaderService(_unitOfWork).Find(JobOrderLine.JobOrderHeaderId);
            JobOrderLine.Rate = Rate;
            JobOrderLine.Amount = Rate * JobOrderLine.DealQty;
            new JobOrderLineService(_unitOfWork).Update(JobOrderLine);
            _unitOfWork.Save();
        ...
        public ActionResult Filters(JobRatePenaltyUpdateFilterArgs Fvm)
        {
        ...
        //public ActionResult ConfirmProdOrderList(List<JobRatePenaltyUpdateViewModel> Selected, int id)
        //{
        ...
        //    if (!BarCodesBased)
        //        return Json(new { Success = "URL", Data = "/JobRatePenaltyUpdate/Create/" + id.ToString() }, JsonRequestBehavior.AllowGet);
        //    else
        //        return Json(new { Success = "URL", Data = "/JobRatePenaltyUpdate/FirstBarCode/" + id.ToString() }, JsonRequestBehavior.AllowGet);
        //}
        ...
        //{
        //    List<JobRatePenaltyUpdateViewModel> ProdOrders = (List<JobRatePenaltyUpdateViewModel>)System.Web.HttpContext.Current.Session["BarCodesWeavingWizardProdOrder"];

        //    JobRatePenaltyUpdateMasterDetailModel vm = new JobRatePenaltyUpdateMasterDetailModel();
        ...
        //    vm.JobRatePenaltyUpdateViewModel = ProdOrders;
        ...
        //[HttpPost]
        //public ActionResult FirstBarCodePost(JobRatePenaltyUpdateMasterDetailModel vm)
        //{
        //    JobRatePenaltyUpdateMasterDetailModel MasterDetailModel = new JobRatePenaltyUpdateMasterDetailModel();
        //    List<JobRatePenaltyUpdateViewModel> List = new List<JobRatePenaltyUpdateViewModel>();

        //    foreach (var item in vm.JobRatePenaltyUpdateViewModel)
        //    {

        //        JobRatePenaltyUpdateViewModel Wizard = new JobRatePenaltyUpdateViewModel();
        ...
        //    MasterDetailModel.JobRatePenaltyUpdateViewModel = List;
        ...
        //[ValidateAntiForgeryToken]
        //[HttpPost]
        //public ActionResult BarCodeSequencePost(JobRatePenaltyUpdateMasterDetailModel vm)
        //{

        //    System.Web.HttpContext.Current.Session["BarCodesWeavingWizardProdOrder"] = vm.JobRatePenaltyUpdateViewModel;

        //    return RedirectToAction("Create", new { id = vm.JobRatePenaltyUpdateViewModel.FirstOrDefault().DocTypeId });
        ...
        //    var ProdOrderLin = ((List<JobRatePenaltyUpdateViewModel>)System.Web.HttpContext.Current.Session["BarCodesWeavingWizardProdOrder"]).FirstOrDefault();
        ...
        //    var Lines = (List<JobRatePenaltyUpdateViewModel>)System.Web.HttpContext.Current.Session["BarCodesWeavingWizardProdOrder"];
        ...
        //    List<JobRatePenaltyUpdateViewModel> ProdOrdersAndQtys = (List<JobRatePenaltyUpdateViewModel>)System.Web.HttpContext.Current.Session["BarCodesWeavingWizardProdOrder"];
    ...
    public class JobRatePenaltyUpdateFilterArgs
    {
