...
namespace Jobs.Areas.Rug.Controllers
{
    ...
    public class SaleInvoiceLineController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult DeletePost(SaleInvoiceLineViewModel vm)
        {
            ...
            StockId = SaleDispatchLine.StockId;
        ...
        public JsonResult GetSaleInvoiceDetail(int SaleInvoiceHeaderId)
        {
            ...
            var temp = (from L in db.SaleInvoiceLine
                        join SDL in db.SaleDispatchLine on L.SaleDispatchLineId equals SDL.SaleDispatchLineId into SDLTable
                        from SDLTab in SDLTable.DefaultIfEmpty()
                        join PL in db.PackingLine on SDLTab.PackingLineId equals PL.PackingLineId into PLTable
                        from PLTab in PLTable.DefaultIfEmpty()
                        join SOL in db.SaleOrderLine on PLTab.SaleOrderLineId equals SOL.SaleOrderLineId into SOLTable
                        from SOLTab in SOLTable.DefaultIfEmpty()
                        join SEL in db.SaleEnquiryLine on SOLTab.ReferenceDocLineId equals SEL.SaleEnquiryLineId into SELTable
                        from SELTab in SELTable.DefaultIfEmpty()
                        join SELE in db.SaleEnquiryLineExtended on SELTab.SaleEnquiryLineId equals SELE.SaleEnquiryLineId into SELETable
                        from SELETab in SELETable.DefaultIfEmpty()
                        where L.SaleInvoiceHeaderId == SaleInvoiceHeaderId
                        group new { L, SELETab } by new { SELETab.BuyerSpecification3, SELETab.BuyerSpecification, L.RateRemark } into Result
                        orderby Result.Key.BuyerSpecification3, Result.Key.BuyerSpecification, Result.Key.RateRemark
                        select new
                        {
                            UniqueName = Result.Key.BuyerSpecification3+Result.Key.BuyerSpecification + (Result.Key.RateRemark ?? ""),
                            QualityName = Result.Key.BuyerSpecification3,
                            ProductGroupId = Result.Key.BuyerSpecification,
                            DesignName = Result.Key.BuyerSpecification,
                            Remark = Result.Key.RateRemark,
                            Rate= Result.Max(m=>m.L.Rate)
                        }).ToList();
            return Json(new { data = temp }, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public void UpdateRatesDesignWise(int SaleInvoiceHeaderId, string QualityName, string DesignName, string Remark, Decimal? Rate)
        {
            if (Rate != null)
            {
                IEnumerable<SaleInvoiceLine> SaleInvoiceLineList = (from L in db.SaleInvoiceLine
                                                                    ...
                                                                    join SELE in db.SaleEnquiryLineExtended on SELTab.SaleEnquiryLineId equals SELE.SaleEnquiryLineId into SELETable
                                                                    from SELETab in SELETable.DefaultIfEmpty()
                                                                    where L.SaleInvoiceHeaderId == SaleInvoiceHeaderId && SELETab.BuyerSpecification3 == QualityName && SELETab.BuyerSpecification == DesignName && (L.RateRemark ?? "") == Remark
                                       select L).ToList();
