...
namespace Jobs.Areas.Rug.Controllers
{
    ...
    public class PackingHeaderWithBuyerDetailController : System.Web.Mvc.Controller
    {
        ...
        public PackingHeaderWithBuyerDetailController(IPackingHeaderService PackingHeaderService, ISaleInvoiceHeaderService SaleInvoiceHeaderService, ISaleDispatchHeaderService SaleDispatchHeaderService, IActivityLogService ActivityLogService, IStockService StockService, IUnitOfWork unitOfWork, IExceptionHandlingService exec)
        {
        ...
        public ActionResult Create(int id)
        {
            ...
            PackingSetting temp = new PackingSettingService(_unitOfWork).GetPackingSettingForDocument(id, p.DivisionId, p.SiteId);
            p.DocTypeId = id;
        ...
        public ActionResult Create(PackingHeaderViewModel svm)
        {
            ...
            PackingSetting Setting = new PackingSettingService(_unitOfWork).GetPackingSettingForDocument(svm.DocTypeId, svm.DivisionId, svm.SiteId);
            ...
            if (ModelState.IsValid && (TimePlanValidation || Continue))
            {
                ...
                if (svm.PackingHeaderId == 0)
                {
                    ...
                    if (Setting.isPostedInSaleInvoice == true)
                    {

                        //SaleInvoiceHeader S = _SaleInvoiceHeaderService.FindLastSaleInvoice(svm.BuyerId);
                        ...
                        saleinvoiceheaderdetail.BLDate = svm.DocDate;
                        saleinvoiceheaderdetail.CircularDate = svm.DocDate;
                        saleinvoiceheaderdetail.OrderDate = svm.DocDate;
                        ...
                        saledispatchheader.SiteId = svm.SiteId;
                        saledispatchheader.DivisionId = svm.DivisionId;
                        ...
                        saledispatchheader.DocNo = svm.DocNo;
                        saledispatchheader.DocDate = svm.DocDate;
                        saledispatchheader.Remark = svm.Remark;
                        saledispatchheader.SaleToBuyerId = svm.BuyerId;
                        saledispatchheader.DeliveryTermsId = new DeliveryTermsService(_unitOfWork).GetDeliveryTermsByName("C & F").DeliveryTermsId;
                        //saledispatchheader.ShipMethodId = new ShipMethodService(_unitOfWork).GetShipMethodByName("BY SEA").;
                        saledispatchheader.ShipMethodId = packingheader.ShipMethodId;
                        saledispatchheader.PackingHeaderId = packingheader.PackingHeaderId;
                        saledispatchheader.StockHeaderId = packingheader.StockHeaderId;
        ...
        public ActionResult Delete(ReasonViewModel vm)
        {
            if (ModelState.IsValid)
            {
                ...
                PackingSetting Setting = new PackingSettingService(_unitOfWork).GetPackingSettingForDocument(PackingHeader.DocTypeId, PackingHeader.DivisionId, PackingHeader.SiteId);
                ...
                foreach (var item in PackingLine)
                {
                    ...
                    if (Setting.isPostedInSaleInvoice == true)
                    {
                        SaleDispatchLine saledispatchline = db.SaleDispatchLine.Where(m => m.PackingLineId == item.PackingLineId).FirstOrDefault();
                        ...
                        new SaleInvoiceLineService(_unitOfWork).Delete(saleinvoiceline.SaleInvoiceLineId);
                        new SaleDispatchLineService(_unitOfWork).Delete(saledispatchline.SaleDispatchLineId);
                ...
                if (Setting.isPostedInSaleInvoice == true)
                {
                    ...
                    SaleDispatchHeader saledispatchheader = db.SaleDispatchHeader.Where(m => m.PackingHeaderId == vm.id).FirstOrDefault();
                    SaleInvoiceHeader SH = db.SaleInvoiceHeader.Where(m => m.SaleDispatchHeaderId == saledispatchheader.SaleDispatchHeaderId).FirstOrDefault();


                    new SaleInvoiceHeaderService(_unitOfWork).Delete(SH.SaleInvoiceHeaderId);
                    new SaleDispatchHeaderService(_unitOfWork).Delete(saledispatchheader.SaleDispatchHeaderId);
