            ...
            if (ModelState.IsValid && BeforeSave && !EventException && (TimePlanValidation || Continue))
            {
                ...
                if (vm.JobInvoiceReturnHeaderId <= 0)
                {
                    if (vm.Nature == TransactionNatureConstants.Return)
                    {
                        ...
                        GoodsRet.DocTypeId = settings.JobReturnDocTypeId ?? 0;
        ...
        public ActionResult Submitted(int Id, string IndexType, string UserRemark, string IsContinue, string GenGatePass)
        {
            ...
            if (ModelState.IsValid && BeforeSave && !EventException)
            {
                if (User.Identity.Name == pd.ModifiedBy || UserRoles.Contains("Admin"))
                {
                    ...
                    LedgerHeaderViewModel.Narration = _JobInvoiceReturnHeaderService.GetNarration(pd.JobInvoiceReturnHeaderId);
...
SELECT	H.JobInvoiceReturnHeaderId,H.DocTypeId,H.DocNo,DocIdCaption+' No' AS DocIdCaption ,
H.SiteId,H.DivisionId,H.DocDate,DTS.DocIdCaption +' Date' AS DocIdCaptionDate ,	
NULL  AS PartyDocNo,	DTS.PartyCaption +' Doc No' AS PartyDocCaption,NULL  AS PartyDocDate,
DTS.PartyCaption +' Doc Date' AS PartyDocDateCaption,JIH.CreditDays,H.Remark,DT.DocumentTypeShortName,	
...
P.Mobile AS PartyMobileNo,	SPR.*,
   ...
   --Receive Lines
	...
	LEFT JOIN web.JobInvoiceReturnLines Rt ON rt.JobInvoiceReturnHeaderId = H.JobInvoiceReturnHeaderId
  	LEFT JOIN Web.JobInvoiceLines L WITH (Nolock) ON L.JobInvoiceLineId=rt.JobInvoiceLineId
  	LEFT JOIN web.JobInvoiceHeaders JIH ON JIH.JobInvoiceHeaderId = L.JobInvoiceHeaderId
  	LEFT JOIN Web.Currencies CUR WITH (Nolock) ON CUR.Id=JIH.CurrencyId
	LEFT JOIN Web.JobReceiveLines JRL WITH (Nolock) ON JRL.JobReceiveLineId=L.JobReceiveLineId
	...
	LEFT JOIN web.ChargeGroupProducts CGPD WITH (Nolock) ON L.SalesTaxGroupProductId = CGPD.ChargeGroupProductId	
   	WHERE H.JobInvoiceReturnHeaderId=" + item + @"
   	ORDER BY L.Sr";
...
SET @Qry = '
		...
		FROM web.jobInvoiceReturnheadercharges H
...
LEFT JOIN Web.JobInvoiceReturnLineCharges LC ON L.JobInvoiceReturnLineId = LC.LineTableId 
LEFT JOIN web.jobInvoiceReturnheaders H ON H.JobInvoiceReturnHeaderId = L.JobInvoiceReturnHeaderId
LEFT JOIN Web.Processes PS WITH (Nolock) ON PS.ProcessId=H.ProcessId
...
WHERE L.JobInvoiceReturnHeaderId =" + item + @" 
