...
{
    [Authorize]
    public class ProductUIDExchangeController : System.Web.Mvc.Controller
    {
        ...
        public ProductUIDExchangeController(IStockHeaderService PurchaseOrderHeaderService, IActivityLogService ActivityLogService, IUnitOfWork unitOfWork, IExceptionHandlingService exec, IStockLineService Stock)
        {
        ...
        public ActionResult Post(StockHeaderViewModel svm)
        {
            ...
            if (ModelState.IsValid && BeforeSave && !EventException && (TimePlanValidation || Continue))
            {
                ...
                if (svm.StockHeaderId <= 0)
                {
                    ...
                    return RedirectToAction("Modify", "ProductUIDExchange", new { Id = s.StockHeaderId }).Success("Data saved successfully");
            ...
            if (ModelState.IsValid && !EventException)
            {
                ...
                if (svm.StockLineId <= 0)
                {
                    ...
                    if (svm.Issue == false)
                    {
                        if (svm.LotNo != null)
                        {
                            //ProductUid PU1 = db.ProductUid.Where(m => m.ProductUIDId == svm.ProductUidId).FirstOrDefault();

                            StockViewModel.LotNo = svm.LotNo + "E";

                            ProductUid PU = new ProductUid();
                            PU.ProductUIDId = -1;
                            PU.ProductUidName = svm.LotNo + "E";
                            PU.ProductId = svm.ProductId;
                            PU.GenDocId = svm.StockHeaderId;
                            PU.GenLineId = s.StockLineId;
                            PU.GenDocNo = temp.DocNo;
                            PU.GenDocTypeId = temp.DocTypeId;
                            PU.GenDocDate = temp.DocDate;
                            PU.LastTransactionDocId = svm.StockHeaderId;
                            PU.LastTransactionLineId = svm.StockLineId;
                            PU.LastTransactionDocNo = temp.DocNo;
                            PU.LastTransactionDocTypeId = temp.DocTypeId;
                            PU.LastTransactionDocDate = temp.DocDate;
                            PU.LastTransactionPersonId = temp.PersonId;
                            PU.CurrenctGodownId = temp.GodownId;
                            PU.CurrenctProcessId = svm.FromProcessId;
                            PU.Status = "Receive";
                            PU.IsActive = true;
                            PU.SaleOrderLineId = svm.SaleOrderLineId;

                            PU.CreatedBy = temp.CreatedBy;
                            PU.CreatedDate = DateTime.Now;
                            PU.ModifiedBy = temp.ModifiedBy;
                            PU.ModifiedDate = DateTime.Now;

                            PU.ObjectState = Model.ObjectState.Added;
                            db.ProductUid.Add(PU);

                            s.ProductUidId = PU.ProductUIDId;
                            //StockViewModel.ProductUidId = PU.ProductUIDId;
                        }
                    }
        ...
        public JsonResult GetSaleOrderLines(string searchTerm, int pageSize, int pageNum, int filter)//filter:ProductId
        {
            var temp = new SaleOrderLineService(_unitOfWork).GetSaleOrderLineForProduct(filter, searchTerm).Skip(pageSize * (pageNum - 1)).Take(pageSize).ToList();

            var count = new SaleOrderLineService(_unitOfWork).GetSaleOrderLineForProduct(filter, searchTerm).Count();
