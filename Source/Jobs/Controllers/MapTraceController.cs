...
{
    [Authorize]
    public class MapTraceController : System.Web.Mvc.Controller
    {
        ...
        public MapTraceController(IProductService ProductService, IFinishedProductService FinishedProductService, IUnitOfWork unitOfWork, IExceptionHandlingService exec)
        {
        ...
        public void PrepareViewBag()
        {
            ...
            temp.Add(new SelectListItem { Text = "Quarter", Value = "Quarter" });
        ...
        {            
            ...
            {
                ProductGroup PG = new ProductGroupService(_unitOfWork).Find(pvm.ProductGroupId);

                if (pvm.ProductId <= 0)
                {

                    FinishedProduct pro = new FinishedProduct();
                    pro.ProductName = pvm.ProductName;
                    pro.ProductCode = pvm.ProductCode;
                    pro.ProductDescription = pvm.ProductDescription;
                    pro.ProductQualityId = pvm.ProductQualityId;
                    pro.ProductDesignId = pvm.ProductDesignId;
                    pro.ColourId = pvm.ColourId;
                    pro.ProductCategoryId = pvm.ProductCategoryId;
                    pro.MapType = pvm.MapType;
                    pro.ProductGroupId = pvm.ProductGroupId;
                    pro.BaseProductGroupId = pvm.BaseProductGroupId;
                    pro.BaseProductSizeId = pvm.BaseProductSizeId;
                    pro.DivisionId = pvm.DivisionId;
                    pro.IsActive = true;
                    ...
                    string MMode;
                    CreateUnitConversion(pvm.MapType,(byte)UnitConversionFors.Standard, (int)pvm.BaseProductSizeId, pro.ProductId, UnitConstants.SqYard, out ManufacturingUnit, out MMode);
                    if (MMode == "Create")
                    {
                    ...
                    return RedirectToAction("Create", new { id = PG.ProductTypeId }).Success("Data saved successfully");
                }
                else
                {
                    ...
                    pt.ColourId = pvm.ColourId;                  
                    pt.ProductCategoryId = pvm.ProductCategoryId;
                    pt.MapType = pvm.MapType;
                    pt.BaseProductGroupId = pvm.BaseProductGroupId;
                    pt.BaseProductSizeId = pvm.BaseProductSizeId;
                    ...
                    UnitConversion ManufacturingUnit;
                    string MMode;
                    CreateUnitConversion(pvm.MapType, (byte)UnitConversionFors.Standard, (int)pvm.BaseProductSizeId, pvm.ProductId, UnitConstants.SqYard, out ManufacturingUnit, out MMode);
                    if (MMode == "Create")
                    {
                        new UnitConversionService(_unitOfWork).Create(ManufacturingUnit);
                    }
                    ...
                    return RedirectToAction("Index", new { id = PG.ProductTypeId }).Success("Data saved successfully");
        ...
        public void CreateUnitConversion(string Type, byte UnitConversionFor, int SizeId, int ProductId, string ToUnit, out UnitConversion UnitConvr, out string Mode)
        {
            ...
            if (SizeExist != null)
            {
                ...
                if (ToUnit == UnitConstants.SqYard)
                {
                    using (SqlConnection sqlConnection = new SqlConnection((string)System.Web.HttpContext.Current.Session["DefaultConnectionString"]))
                    {
                        ...
                        if (Type == "Quarter")
                            SizeExist.ToQty = SizeExist.ToQty/4;
                        else if (Type == "Half")
                            SizeExist.ToQty = SizeExist.ToQty/2;
            ...
            else
            {
                ...
                if (ToUnit == UnitConstants.SqYard)
                {
                    using (SqlConnection sqlConnection = new SqlConnection((string)System.Web.HttpContext.Current.Session["DefaultConnectionString"]))
                    {
                        ...
                        if (Type == "Quarter")
                            UnitConv.ToQty = UnitConv.ToQty / 4;
                        else if (Type == "Half")
                            UnitConv.ToQty = UnitConv.ToQty / 2;
        ...
        public ActionResult Edit(int id)
        {
            ...
            PrepareViewBag();
            ProductViewModel pt = _ProductService.GetFinishedProduct(id);
            if (pt == null)
            {
                return HttpNotFound();
            }
            ProductGroup PG = new ProductGroupService(_unitOfWork).Find(pt.ProductGroupId);
            ViewBag.id = PG.ProductTypeId;
        ...
        public JsonResult GetMapTraceName(int ProductGroupId, int ProductQualityId, int BaseProductGroupId, int BaseProductSizeId, int ColourId, String Type)
        {
            ProductGroup PG1 = db.ProductGroups.Where(m => m.ProductGroupId == ProductGroupId).FirstOrDefault();
            ProductQuality PQ = db.ProductQuality.Where(m => m.ProductQualityId == ProductQualityId).FirstOrDefault();
            ProductGroup PG = db.ProductGroups.Where(m => m.ProductGroupId == BaseProductGroupId).FirstOrDefault();
            Size S = db.Size.Where(m => m.SizeId == BaseProductSizeId).FirstOrDefault();
            Colour C = db.Colour.Where(m => m.ColourId == ColourId).FirstOrDefault();

            MapTraceName Mapname = new MapTraceName();
            Mapname.ProductName = PQ.ProductQualityName + "-" + PG.ProductGroupName + "-" + S.SizeName + "-" + C.ColourName + "-"+ Type + "-" +PG1.ProductGroupName;
            Mapname.ProductCode = Mapname.ProductName.Length > 50 ? Mapname.ProductName.Substring(0, 44) + "-" + PG1.ProductGroupName : Mapname.ProductName;
            return Json(Mapname);
        }

        public class MapTraceName
        {
