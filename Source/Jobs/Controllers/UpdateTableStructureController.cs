...
namespace Module
{
    ...
    public class UpdateTableStructureController : Controller
    {
        ...
        public ActionResult UpdateTables()
        {
            ...
            try
            {
                if ((int)ExecuteScaler("SELECT Count(*) AS Cnt FROM INFORMATION_SCHEMA.Tables WHERE TABLE_NAME = 'DocumentTypeTimeExtensionRequests'") == 0)
                {
                    mQry = @"CREATE TABLE Web.DocumentTypeTimeExtensionRequests
	                            (
	                            DocumentTypeTimeExtensionRequestId INT IDENTITY NOT NULL,
	                            DocTypeId                   INT,
	                            ...
	                            ModifiedDate                DATETIME NOT NULL,
	                            IsApproved                  BIT,
	                            CONSTRAINT [PK_Web.DocumentTypeTimeExtensionRequests] PRIMARY KEY (DocumentTypeTimeExtensionRequestId),
	                            CONSTRAINT [FK_Web.DocumentTypeTimeExtensionRequests_Web.Divisions_DivisionId] FOREIGN KEY (DivisionId) REFERENCES Web.Divisions (DivisionId),
	                            CONSTRAINT [FK_Web.DocumentTypeTimeExtensionRequests_Web.DocumentTypes_DocTypeId] FOREIGN KEY (DocTypeId) REFERENCES Web.DocumentTypes (DocumentTypeId),
	                            CONSTRAINT [FK_Web.DocumentTypeTimeExtensionRequests_Web.Sites_SiteId] FOREIGN KEY (SiteId) REFERENCES Web.Sites (SiteId)
	                            )
                            

                            CREATE INDEX [IX_DocTypeId]	ON [Web.DocumentTypeTimeExtensionRequests] (DocTypeId)
                            CREATE INDEX [IX_DivisionId] ON [Web.DocumentTypeTimeExtensionRequests] (DivisionId)
                            CREATE INDEX [IX_SiteId] ON [Web.DocumentTypeTimeExtensionRequests] (SiteId)
            ...
            try
            {
                if ((int)ExecuteScaler("SELECT Count(*) AS Cnt FROM INFORMATION_SCHEMA.Tables WHERE TABLE_NAME = 'PersonGodowns'") == 0)
                {
                    mQry = @"	CREATE TABLE Web.PersonGodowns
	                            (
	                            PersonGodownId INT IDENTITY NOT NULL,
	                            DivisionId                  INT NOT NULL,
	                            SiteId                      INT NOT NULL,
                                PersonId                    INT NOT NULL,
	                            ProductCategoryId  			INT,
	                            GodownId                    INT,
	                            GodownCode                  NVARCHAR (max),
	                            CreatedBy                   NVARCHAR (max),
	                            ModifiedBy                  NVARCHAR (max),
	                            CreatedDate                 DATETIME NOT NULL,
	                            ModifiedDate                DATETIME NOT NULL,
	                            CONSTRAINT [PK_Web.PersonGodowns] PRIMARY KEY (PersonGodownId),
	                            CONSTRAINT [FK_Web.PersonGodowns_Web.Divisions_DivisionId] FOREIGN KEY (DivisionId) REFERENCES Web.Divisions (DivisionId),
	                            CONSTRAINT [FK_Web.PersonGodowns_Web.Persons_PersonId] FOREIGN KEY (PersonId) REFERENCES Web.People (PersonId),
	                            CONSTRAINT [FK_Web.PersonGodowns_Web.ProductCategories_ProductCategoryId] FOREIGN KEY (ProductCategoryId) REFERENCES Web.ProductCategories (ProductCategoryId),
	                            CONSTRAINT [FK_Web.PersonGodowns_Web.Godowns_DocTypeId] FOREIGN KEY (GodownId) REFERENCES Web.Godowns (GodownId),
	                            CONSTRAINT [FK_Web.PersonGodowns_Web.Sites_SiteId] FOREIGN KEY (SiteId) REFERENCES Web.Sites (SiteId)
	                            )
                            

                                CREATE INDEX [IX_DivisionId] ON Web.PersonGodowns (DivisionId)
                                CREATE INDEX [IX_SiteId] ON Web.PersonGodowns (SiteId)
                                CREATE INDEX [IX_PersonId] ON Web.PersonGodowns (PersonId)
                                CREATE INDEX [IX_GodownId] ON Web.PersonGodowns (GodownId)
							    CREATE INDEX [IX_ProductCategoryId] ON Web.PersonGodowns (ProductCategoryId)
            ...
            try
            {
                if ((int)ExecuteScaler("SELECT Count(*) AS Cnt FROM INFORMATION_SCHEMA.Tables WHERE TABLE_NAME = 'PersonBlockedDocumentTypes'") == 0)
                {
                    mQry = @"CREATE TABLE Web.PersonBlockedDocumentTypes
	                        (
	                        PersonBlockedDocumentTypeId      INT IDENTITY NOT NULL,
	                        PersonID               INT NOT NULL,
	                        BlockedDocumentTypeId  INT NOT NULL,
	                        OMSId                  NVARCHAR (50),
	                        CONSTRAINT [PK_Web.PersonBlockedDocumentTypes] PRIMARY KEY (PersonBlockedDocumentTypeId) WITH (FILLFACTOR = 90),
	                        CONSTRAINT [FK_Web.PersonBlockedDocumentTypes.BlockedDocumentTypeId] FOREIGN KEY (BlockedDocumentTypeId) REFERENCES Web.DocumentTypes (DocumentTypeId),
	                        CONSTRAINT [FK_Web.PersonBlockedDocumentTypes.PersonID] FOREIGN KEY (PersonID) REFERENCES Web.People (PersonID)
	                        )
                            ";
                    ExecuteQuery(mQry);
                }
...
CREATE INDEX [IX_DocumentTypeHeaderAttributeId]
	...
	WITH (FILLFACTOR = 90)
            ...
            AddFields("Sites", "Sr", "INT");
            AddFields("Divisions", "Sr", "INT");
            ...
            AddFields("FinishedProduct", "BaseProductGroupId", "INT", "ProductGroups");
            AddFields("FinishedProduct", "BaseProductSizeId", "INT", "Sizes");
            AddFields("FinishedProduct", "IsConsumptionPcsWise", "Bit");

            AddFields("Departments", "ParentDepartmentId", "INT", "Departments");

            AddFields("Users", "IsActive", "Bit");
            ...
            AddFields("DocumentTypeTimeExtensions", "ReferenceDocId", "Int");
            ...
            AddFields("SaleEnquirySettings", "isVisiblePersonProductUid", "BIT");
            ...
            AddFields("StockHeaderSettings", "isVisibleProdOrder", "BIT");
            ...
            AddFields("PersonSettings", "DefaultTdsCategoryId", "Int", "TdsCategory");
            AddFields("PersonSettings", "DefaultTdsGroupId", "Int", "TdsGroup");
            AddFields("PersonSettings", "DefaultSalesTaxGroupPartyId", "Int", "ChargeGroupPerson");
