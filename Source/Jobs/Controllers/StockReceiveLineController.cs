...
namespace Jobs.Controllers
{
    ...
    public class StockReceiveLineController : System.Web.Mvc.Controller
    {
        ...
        public JsonResult GetProductDetailJson(int ProductId, int StockId, string ProductUidIdName)
        {
            ProductViewModel product = new ProductService(_unitOfWork).GetProduct(ProductId);

            ProductUid PU = new ProductUidService(_unitOfWork).FGetProductUidByName(ProductUidIdName);

            StockHeader  SH = new StockHeaderService(_unitOfWork).Find(StockId);
            StockHeaderSettings Setting = new StockHeaderSettingsService(_unitOfWork).GetStockHeaderSettingsForDocument(SH.DocTypeId, SH.DivisionId, SH.SiteId);

            Decimal Qty = 0;

            if (Setting.isPostedInStockProcess == true)
            {
                if (PU != null)
                    Qty = _StockLineService.ProductStockProcessBalance(ProductId, StockId, PU.ProductUIDId);
                else
                    Qty = _StockLineService.ProductStockProcessBalance(ProductId, StockId, 0);
            }
        ...
        {
            var Query = _StockLineService.GetProdOrderHelpListForProduct(filter, searchTerm);
