...
namespace Jobs.Controllers
{
    ...
    [Authorize]
    public class LedgerLine_WithCalculationController : System.Web.Mvc.Controller
    {
        ...
        public LedgerLine_WithCalculationController(ILedgerService SaleOrder, IUnitOfWork unitOfWork, IExceptionHandlingService exec)
        {
        ...
        public ActionResult _CreatePost(LedgersViewModel svm)
        {
            ...
            if (ModelState.IsValid && BeforeSave && !EventException)
            {
                if (svm.LedgerLineId <= 0)
                {
                    ...
                    LedgerLine.Qty = svm.Qty;
                    LedgerLine.DealQty = svm.DealQty;
                    LedgerLine.Rate = svm.Rate;
                    LedgerLine.Specification = svm.Specification;
                    ...
                    if (svm.linecharges != null)
                        ...
                        {
                            item.LineTableId = LedgerLine.LedgerLineId;
                            //item.PersonID = header.LedgerAccountId;
                            item.HeaderTableId = header.LedgerHeaderId;
                            item.ObjectState = Model.ObjectState.Added;
                            db.LedgerLineCharge.Add(item);
                            //new LedgerLineChargeService(_unitOfWork).Create(item);
                        }
                    ...
                    if (svm.footercharges != null)
                        foreach (var item in svm.footercharges)
                        {
                            ...
                            if (item.Id > 0)
                            {
                                ...
                                var footercharge = new LedgerHeaderChargeService(_unitOfWork).Find(item.Id);
                                footercharge.Rate = item.Rate;
                                footercharge.Amount = item.Amount;
                                footercharge.ObjectState = Model.ObjectState.Modified;
                                db.LedgerHeaderCharges.Add(footercharge);
                                //new LedgerHeaderChargeService(_unitOfWork).Update(footercharge);
                            ...
                            else
                            {
                                ...
                                //item.PersonID = header.LedgerAccountId;
                                item.ObjectState = Model.ObjectState.Added;
                                db.LedgerHeaderCharges.Add(item);
                                //new LedgerHeaderChargeService(_unitOfWork).Create(item);
                            }
                        }
                ...
                else //Edit Mode 
                {
                    ...
                    if (svm.linecharges != null)
                        foreach (var item in svm.linecharges)
                        {
                            var productcharge = new LedgerLineChargeService(_unitOfWork).Find(item.Id);
                            LedgerLineCharge ExRecLine = new LedgerLineCharge();
                            ExRecLine = Mapper.Map<LedgerLineCharge>(productcharge);

                            productcharge.Rate = item.Rate;
                            productcharge.Amount = item.Amount;
                            productcharge.ObjectState = Model.ObjectState.Modified;
                            db.LedgerLineCharge.Add(productcharge);
                    ...
                    if (svm.footercharges != null)
                        ...
                        {
                            var footercharge = new LedgerHeaderChargeService(_unitOfWork).Find(item.Id);
                            LedgerHeaderCharge ExRecLine = new LedgerHeaderCharge();
                            ExRecLine = Mapper.Map<LedgerHeaderCharge>(footercharge);
        ...
        public ActionResult DeletePost(LedgersViewModel vm)
        {
            ...
            if (BeforeSave && !EventException)
            {
                ...
                var chargeslist = (from p in db.LedgerLineCharge
                                   where p.LineTableId == vm.LedgerLineId
                                   select p).ToList();

                if (chargeslist != null)
                    foreach (var item1 in chargeslist)
                    {
                        item1.ObjectState = Model.ObjectState.Deleted;
                        db.LedgerLineCharge.Remove(item1);
                ...
                if (vm.footercharges != null)
                    ...
                    {
                        var footer = new LedgerHeaderChargeService(_unitOfWork).Find(item.Id);
                        footer.Rate = item.Rate;
                        footer.Amount = item.Amount;
                        footer.ObjectState = Model.ObjectState.Modified;
                        db.LedgerHeaderCharges.Add(footer);
                    }
