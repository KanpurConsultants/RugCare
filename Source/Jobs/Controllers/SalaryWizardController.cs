...
namespace Jobs.Controllers
{
    ...
    public class SalaryWizardController : System.Web.Mvc.Controller
    {
        ...
        [HttpGet]
        public ActionResult SalaryWizard(int id, int? SalaryHeaderId, string Message ="")
        {
        ...
        public string Post(List<SalaryWizardResultViewModel> SalaryDataList)
        {
            ...
            {
                if (SalaryData.Advance > SalaryData.XAdvance)
                {
                    ErrorMsg = ErrorMsg + " Advance is Invalid For " + SalaryData.EmployeeName;
                }

                if (SalaryData.LoanEMI > SalaryData.XLoanEMI)
                {
                    ErrorMsg = ErrorMsg + " LoanEMI is Invalid For " + SalaryData.EmployeeName;
                }

                if (SalaryData.Additions > SalaryData.XAdditions)
                {
                    ErrorMsg = ErrorMsg + " Additions is Invalid For " + SalaryData.EmployeeName;
                }

                if (SalaryData.Deductions > SalaryData.XDeductions)
                {
                    ErrorMsg = ErrorMsg + " Deductions is Invalid For " + SalaryData.EmployeeName;
                }
            }
            ...
            else
            {
                ...
                return (string)System.Configuration.ConfigurationManager.AppSettings["JobsDomain"] + "/SalaryWizard/SalaryWizard/" + SalaryDataList.FirstOrDefault().DocTypeId+ "?Message="+ ErrorMsg;
