                        ...
                        if (item.SqlParameter != "" && item.SqlParameter != null)
                        {
                            if (item.SqlParameter == "@LoginSite" || item.SqlParameter == "@LoginDivision")
                            {
                                if (item.SqlParameter == "@LoginSite")
                                    ...
                                    cmd.Parameters.AddWithValue(item.SqlParameter, (int)System.Web.HttpContext.Current.Session[SessionNameConstants.LoginDivisionId]); ReportFilters.Add(item.DisplayName, form[item.FieldName].ToString());

                            }
                            else if (item.FieldName == "Site" && form[item.FieldName].ToString() == "")
                            {
                                cmd.Parameters.AddWithValue(item.SqlParameter, (int)System.Web.HttpContext.Current.Session[SessionNameConstants.LoginSiteId]); ReportFilters.Add(item.DisplayName, form[item.FieldName].ToString());
                            }
                            ...
                            else
                            {
                                    if (form[item.FieldName].ToString() != "")
                                    {
                                        ...
                                        {
                                            cmd.Parameters.AddWithValue(item.SqlParameter, (form[item.FieldName].ToString() != "" ? String.Format("{0:MMMM dd yyyy}", form[item.FieldName].ToString()) : "Null")); ReportFilters.Add(item.DisplayName, form[item.FieldName].ToString());

                                    }
                                        else
                                        {
                                            cmd.Parameters.AddWithValue(item.SqlParameter, (form[item.FieldName].ToString() != "" ? form[item.FieldName].ToString() : "Null")); ReportFilters.Add(item.DisplayName, form[item.FieldName].ToString());
