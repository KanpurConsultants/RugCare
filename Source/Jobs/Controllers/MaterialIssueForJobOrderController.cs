...
namespace Jobs.Controllers
{
    ...
    [Authorize]
    public class MaterialIssueForJobOrderController : System.Web.Mvc.Controller
    {
        ...
        public MaterialIssueForJobOrderController(IStockLineService Stock, IUnitOfWork unitOfWork, IExceptionHandlingService exec)
        {
        ...
        public ActionResult _ResultsPost(StockMasterDetailModel vm)
        {
            ...
            foreach (var item in vm.StockLineViewModel)
            {
                if (item.Qty > 0)
                {
                    ...
                    if (Settings.isAllowedNegativeStock == false)
                    {
                        Decimal ES = _StockLineService.GetExcessStock(item.ProductId, item.Dimension1Id, item.Dimension2Id, null, item.LotNo, item.StockHeaderId, Settings.SqlFuncCurrentStock);
                        if (ES < item.Qty)
                        {
        ...
        public ActionResult SetFlagForNegativeStock()
        {
            bool CreateNegativeStock = true;

            return Json(CreateNegativeStock);
        }
        ...
        {            
            JobOrderHeader JOH = db.JobOrderHeader.Find(id);
            if (JOH != null)
            {
                var p = _StockLineService.GetStockLineListForIndex((int)JOH.StockHeaderId).ToList();
                return Json(p, JsonRequestBehavior.AllowGet);
            }
            else
            {
                var p = _StockLineService.GetStockLineListForIndex(0).ToList();
                return Json(p, JsonRequestBehavior.AllowGet);
            }
        ...
        {
            JobOrderHeader JOH = db.JobOrderHeader.Find(Id);
            StockHeader H = db.StockHeader.Find(JOH.StockHeaderId);
        ...
        public ActionResult _CreatePost(StockLineViewModel svm)
        {
            ...
            if (settings != null)
            {
                ...
                if (settings.isVisibleLossPer == true && settings.MaxLossPer != null )
                {
                    if ( svm.LossPer > settings.MaxLossPer )
                    {
                        ModelState.AddModelError("LossPer", "LossPer is Invalid !");
                    }
                }
                ...
                if (settings.isAllowedNegativeStock == false )
                {
                    Decimal ES = _StockLineService.GetExcessStock(svm.ProductId, svm.Dimension1Id, svm.Dimension2Id, null, svm.LotNo, svm.StockHeaderId, settings.SqlFuncCurrentStock);
                    if (ES < svm.Qty)
                    {
                        //ModelState.AddModelError("Qty", "Negative Stock is not Allowed");
                        if (svm.CreateNegativeStock == false)
                        {                            
                            PrepareViewBag(svm);
                            ViewBag.LineMode = "Create";
                            ModelState.AddModelError("Qty", "Negative Stock is not Allowed");
                        ...
                        {
                            Lognarration = "Entry is Done With Negative Stock";
                        }
                    }
            ...
            if (ModelState.IsValid && BeforeSave && !EventException)
            {
                ...
                if (svm.StockLineId <= 0)
                {
                    ...
                    return RedirectToAction("_Create", new { id = temp.ReferenceDocId });
        ...
        public ActionResult GetStockInForProduct(string searchTerm, int pageSize, int pageNum, int filter, int? ProductId, int? Dimension1Id, int? Dimension2Id, int? Dimension3Id, int? Dimension4Id)//DocTypeId
        {
            ...
            if (Dimension1Id == null && Dimension2Id == null && Dimension3Id == null && Dimension4Id == null)
            {  Query = _StockLineService.GetPendingStockInForIssue_WithoutDimension(filter, ProductId, searchTerm); }
            else
            {  Query = _StockLineService.GetPendingStockInForIssue(filter, ProductId, Dimension1Id, Dimension2Id, Dimension3Id, Dimension4Id, searchTerm); }
