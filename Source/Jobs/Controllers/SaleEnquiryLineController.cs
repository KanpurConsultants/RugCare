...
namespace Jobs.Controllers
{
    ...
    public class SaleEnquiryLineController : System.Web.Mvc.Controller
    {
        ...
        public ActionResult _CreatePost(SaleEnquiryLineViewModel svm)
        {
            ...
            if (ModelState.IsValid)
            {
                if (svm.SaleEnquiryLineId <= 0)
                {
                    ...
                    if (svm.FirstPersonProductUid != null && svm.FirstPersonProductUid > 0)
                    {
                        for (int i = 0; i < svm.Qty; i++)
                        {
                            string ProductUidName = (svm.FirstPersonProductUid + i).ToString();
                            PersonProductUid PPU = new PersonProductUid();
                            PPU.GenLineId = s.SaleEnquiryLineId;
                            PPU.ProductUidName = ProductUidName;
                            PPU.GenDocId = temp.SaleEnquiryHeaderId;
                            PPU.GenDocNo = temp.BuyerEnquiryNo;
                            PPU.GenDocTypeId = temp.DocTypeId;
                            PPU.CreatedDate = DateTime.Now;
                ...
                else
                {
                    ...
                    if (SOL!=null)
                    {
                        SOL.DueDate = svm.DueDate;
                        SOL.ProductId = (int)svm.ProductId;
                        SOL.Qty = svm.Qty;
                        SOL.DealQty = svm.DealQty ?? 0;
                        SOL.DealUnitId = svm.DealUnitId;
                        SOL.UnitConversionMultiplier = svm.UnitConversionMultiplier;
                        SOL.Rate = svm.Rate ?? 0;
                        SOL.Amount = svm.Amount ?? 0;
                        SOL.Remark = svm.Remark;
                        SOL.ModifiedDate = DateTime.Now;
                        SOL.ModifiedBy = User.Identity.Name;
                        new SaleOrderLineService(_unitOfWork).Update(SOL);
                    }
        ...
        private ActionResult _Modify(int id)
        {
            ...
            SaleOrderLine SOL = (from L in db.SaleOrderLine where L.ReferenceDocLineId == id && L.ReferenceDocTypeId == H.DocTypeId select L).FirstOrDefault();
            if (SOL != null)
            {
                MaterialPlanForSaleOrder MS = (from L in db.MaterialPlanForSaleOrder where L.SaleOrderLineId == SOL.SaleOrderLineId select L).FirstOrDefault();
                if (MS != null)
                    s.GeneratedSaleOrderLineId = MS.SaleOrderLineId;
                else
        ...
        public ActionResult DeletePost(SaleEnquiryLineViewModel vm)
        {
            ...
            if (SOL != null)
            {
                new SaleOrderLineStatusService(_unitOfWork).Delete(SOL.SaleOrderLineId);
