...
Salary.controller('MainCtrl', ['$scope', '$log', '$http', 'uiGridConstants', 'uiGridExporterConstants', 'uiGridExporterService',
  ...
  function ($scope, $log, $http, uiGridConstants, uiGridExporterConstants, uiGridExporterService) {
      ...
      $scope.Post = function (SaleEnquiryLineId, ProductName) {
          ...
          $.ajax({
              url: "/EmployeeSalaryChangeWizard/Post/",
      ...
      {
          $.ajax({
              url: '/EmployeeSalaryChangeWizard/EmployeeSalaryChangeWizardFill/' + $(this).serialize(),
              type: "POST",
              ...
              success: function (result) {
                  ...
                  if (result.Success == true) {
                      ...
                      $scope.gridOptions.columnDefs.push({ field: 'CurrentSalary', width: 100, cellClass: 'text-right cell-text ', headerCellClass: 'text-right header-text', enableCellEdit: false });
                      $scope.gridOptions.columnDefs.push({ field: 'NewSalary', width: 110, cellClass: 'text-right cell-text ', headerCellClass: 'text-right header-text', enableCellEdit: true });
