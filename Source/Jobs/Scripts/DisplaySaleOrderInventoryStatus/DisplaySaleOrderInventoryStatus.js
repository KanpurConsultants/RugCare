...
SaleOrderInventoryStatusDisplay.controller('MainCtrl', ['$rootScope', '$scope', '$log', '$http', 'uiGridConstants', '$timeout',



  function ($rootScope, $scope, $log, $http, uiGridConstants, uiGridTreeViewConstants, $timeout) {
      $scope.gridOptions = {
          enableGridMenu: true,
      ...
      var init = function () {
            if ($("#NextFormat").val() != '' && $("#NextFormat").val() != null)
            {
                setTimeout(function () {
                    $scope.BindData();
                }, 10000);
      ...
      $scope.ShowDetail = function () {
          ...
          if (IsOpenInNewTab == true)
          {
              ...
              if (rowCol.col.name == "STK") {
                  NextFormat = "Stock Detail";
                  if ($("#ReportType").val() == "Summary")
                      TextHidden = rowCol.row.entity.ProductId;
                  else
                      TextHidden = rowCol.row.entity.SaleOrderLineId;
              }

              if (rowCol.col.name == "LOOM") {
                  NextFormat = "Loom Detail";
                  if ($("#ReportType").val() == "Summary")
                      TextHidden = rowCol.row.entity.ProductId;
                  else
                      TextHidden = rowCol.row.entity.SaleOrderLineId;
              ...
              if (rowCol.col.name == "D_B") {
                  NextFormat = "DB Detail";
                  if ($("#ReportType").val() == "Summary")
                      TextHidden = rowCol.row.entity.ProductId;
                  else
                      TextHidden = rowCol.row.entity.SaleOrderLineId;
              ...
              window.open('/Display_SaleOrderInventoryStatus/Display_SaleOrderInventoryStatus/?MenuId=15846&TextHidden=' + TextHidden + '&NextFormat=' + NextFormat
                   + '&FromDate=' + $("#FromDate").val()
                   + '&ToDate=' + $("#ToDate").val()
                   + '&StatusOnDate=' + $("#StatusOnDate").val()
                   + '&ReportType=' + $("#ReportType").val()
                   + '&ReportFor=' + $("#ReportFor").val()
                   + '&Site=' + $("#Site").val()
                   + '&Division=' + $("#Division").val()
                   + '&Buyer=' + $("#Buyer").val()
                   + '&SaleOrderHeaderId=' + $("#SaleOrderHeaderId").val()
                   + '&Product=' + $("#Product").val()
                   + '&ProductCategory=' + $("#ProductCategory").val()
                   + '&ProductQuality=' + $("#ProductQuality").val()
                   + '&ProductGroup=' + $("#ProductGroup").val()
                   + '&ProductSize=' + $("#ProductSize").val()
                   + '&BuyerDesign=' + $("#BuyerDesign").val(), '_blank ');
          }
          else {
              ...
              success: function (result) {
                  ...
                  if (result.Success == true) {
                      ...
                      else if ($("#ReportType").val() == "Summary" && ($("#NextFormat").val() == "" || $("#NextFormat").val() == null))
                      {
                          ...
                          $scope.gridOptions.columnDefs.push({ field: 'Status', width: 100, cellClass: 'cell-text ', headerCellClass: 'header-text' });
                      }
                          ...
                          $scope.gridOptions.columnDefs.push({ field: 'Slip_No', width: 100, cellClass: 'cell-text ', headerCellClass: 'header-text' });
                          $scope.gridOptions.columnDefs.push({ field: 'Quality', width: 100, cellClass: 'cell-text ', headerCellClass: 'header-text' });
                          $scope.gridOptions.columnDefs.push({ field: 'Design', width: 100, cellClass: 'cell-text ', headerCellClass: 'header-text' });
                          $scope.gridOptions.columnDefs.push({ field: 'Colour', width: 100, cellClass: 'cell-text ', headerCellClass: 'header-text' });
                          $scope.gridOptions.columnDefs.push({ field: 'Size', width: 100, cellClass: 'cell-text ', headerCellClass: 'header-text' });
                      ...
                      else if ($("#NextFormat").val() == "DB Detail") {
                          $scope.gridOptions.columnDefs.push({ field: 'Invoice_No', width: 100, cellClass: 'cell-text ', headerCellClass: 'header-text' });
                          $scope.gridOptions.columnDefs.push({ field: 'Date', width: 100, cellClass: 'cell-text ', headerCellClass: 'header-text' });
                          $scope.gridOptions.columnDefs.push({ field: 'CarpetNo', width: 100, cellClass: 'cell-text ', headerCellClass: 'header-text' });
                          $scope.gridOptions.columnDefs.push({ field: 'Order_No', width: 100, cellClass: 'cell-text ', headerCellClass: 'header-text' });
