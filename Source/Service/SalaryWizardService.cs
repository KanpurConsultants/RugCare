...
namespace Service
{
    public interface ISalaryWizardService : IDisposable
    {
        ...
        IEnumerable<OtherDeductionLedgerIdList> GetOtherDeductionList(int LedgerAccountId);
        IEnumerable<OtherAdditionLedgerIdList> GetOtherAdditionList(int LedgerAccountId);
    }
    ...
    public class SalaryWizardService : ISalaryWizardService
    {
        ...
        public IEnumerable<SalaryWizardResultViewModel> GetSalaryDetail(SalaryWizardViewModel vm)
        {
            ...
            if (vm.SalaryHeaderId == 0)
            {
                ...
                if (vm.WagesPayType == "Jobwork")
                {
                    ...
                    mQry = @"SELECT 0 As SalaryHeaderId, E.PersonID AS EmployeeId, E.Name+','+E.Suffix AS EmployeeName, Convert(int,E.Code) AS Code, 1.00 AS Days, 0.00 as BasicPay, 0.00 AS TDS, 0.00 AS TDSBaseValue, 
                        ...
                        LEFT JOIN (
	                        ...
	                        WHERE H.AdjustmentType = '" + LedgerHeaderAdjustmentTypeConstants.Loan + "'" + @"
                        ...
                        LEFT JOIN (
	                        ...
	                        LEFT JOIN (
		                        ...
		                        GROUP BY La.DrLedgerId
	                        	                        ...
	                        	                        WHERE H.AdjustmentType = '" + LedgerHeaderAdjustmentTypeConstants.Advance + "'" + @"
                            AND IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0) > 0
	                        GROUP BY E.PersonID
                ...
                else if (vm.WagesPayType == "Jobwork_ByVoucher")
                {
                    ...
                    mQry = @"SELECT 0 As SalaryHeaderId, E.PersonID AS EmployeeId, E.Name+','+E.Suffix AS EmployeeName, Convert(int,E.Code) AS Code, 1.00 AS Days, isnull(VSalaryLine.Amount,0) as BasicPay, 0.00 AS TDS, 0.00 AS TDSBaseValue, 0.00 AS Additions, 0.00 AS Deductions, 0.00 AS XAdditions, 0.00 AS XDeductions,
                        0.00 as RetensionAmount, IsNull(VLoan.LoanEMI,0) AS LoanEMI, IsNull(VAdvance.Advance,0) AS Advance,  IsNull(VLoan.LoanEMI,0) AS XLoanEMI, IsNull(VAdvance.Advance,0) AS XAdvance, @DocDate As DocDate, 
                        @DocTypeId As DocTypeId, @WagesPayType AS WagesPayType, @Remark As HeaderRemark, Convert(Decimal(18,4),DAY(EOMONTH(@DocDate))) - IsNull(VSunday.NoOfSundays,0) AS MonthDays
                        FROM Web.People E
                        LEFT JOIN web.DocumentTypes DT ON DT.DocumentTypeId = E.DocTypeId 
                ...
                else if (vm.WagesPayType == "Jobwork_ByInvoice")
                {
                    ...
                    mQry = @"SELECT 0 As SalaryLineId, E.PersonID AS PersonID,  VSalaryLine.DocTypeId AS ReferenceDocTypeId, VSalaryLine.JobInvoiceHeaderId AS ReferenceDocId, VSalaryLine.JobInvoiceLineId ReferenceDocLineId
                                ...
                                LEFT JOIN
                                     (
                                       ...
                                       WHERE 1 = 1 AND H.Status =1 AND SLR.SalaryLineId IS NULL AND H.DocDate <= @DocDate " + 
                                       (vm.ContraDocumentTypeId != null ? " AND H.DocTypeId IN (SELECT Items FROM[dbo].[Split](@ContraDocumentTypeId, ',')) " : "") +
                                       @" ) AS VSalaryLine ON E.PersonID = VSalaryLine.PersonId
                    ...
                    if (CompanyName == "Bhadohi Carpets")
                        ...
                        @DocTypeId As DocTypeId, @WagesPayType AS WagesPayType, @Remark As HeaderRemark, Convert(Decimal(18,4),DAY(EOMONTH(@DocDate))) - IsNull(VSunday.NoOfSundays,0) AS MonthDays
                        FROM Web.People E
                        LEFT JOIN web.BusinessEntities BE WITH (Nolock) ON BE.PersonID = E.PersonID 
                        LEFT JOIN web.TdsGroups  TG WITH (Nolock) ON TG.TdsGroupId = BE.TdsGroupId 
                        LEFT JOIN web.TdsRates TR WITH (Nolock) ON TR.TdsGroupId = TG.TdsGroupId 
                        LEFT JOIN web.DocumentTypes DT ON DT.DocumentTypeId = E.DocTypeId 
                        LEFT JOIN 
                        (
                            SELECT H.ProcessId, L.JobWorkerId AS PersonId, sum(L.Amount) AS Amount , sum(L.RetensionAmount) AS RetensionAmount  
                            FROM web.JobInvoiceHeaders H
                            LEFT JOIN web.JobInvoiceLines L ON L.JobInvoiceHeaderId = H.JobInvoiceHeaderId
                            LEFT JOIN web.SalaryLineReferences SLR ON SLR.ReferenceDocTypeId = H.DocTypeId AND H.JobInvoiceHeaderId = SLR.ReferenceDocId AND L.JobInvoiceLineId = SLR.ReferenceDocLineId
                            WHERE 1 = 1 AND H.Status =1 AND SLR.SalaryLineId IS NULL AND H.DocDate <= @DocDate " +
                            (vm.ContraDocumentTypeId != null ? " AND H.DocTypeId IN (SELECT Items FROM[dbo].[Split](@ContraDocumentTypeId, ',')) " : "") +
                            @" Group By H.ProcessId, L.JobWorkerId
                        ) AS VSalaryLine ON E.PersonID = VSalaryLine.PersonId
                        LEFT JOIN (
                            SELECT Count(*) AS NoOfSundays
                            FROM master..spt_values 
                        ...
                        LEFT JOIN (
                         ...
                         LEFT JOIN 
                        (
	                        SELECT H.ProcessId, E.PersonID, Sum(IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0)) AS DBTAmount, Min(L.LedgerId) As LoanLedgerId
	                        FROM Web.People E
	                        LEFT JOIN Web.LedgerAccounts A ON E.PersonID = A.PersonId
	                        LEFT JOIN Web.Ledgers L ON A.LedgerAccountId = L.LedgerAccountId
	                        LEFT JOIN Web.LedgerLines LL ON L.LedgerLineId = LL.LedgerLineId
	                        ...
	                        WHERE H.AdjustmentType = '" + LedgerHeaderAdjustmentTypeConstants.DebitNote + "'" + @"
                            AND IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0) > 0
	                        GROUP BY H.ProcessId,E.PersonID
                        ) AS VDBT ON E.PersonID = VDBT.PersonID AND VSalaryLine.ProcessId = VDBT.ProcessId
                         LEFT JOIN 
                        (
	                        SELECT H.ProcessId,E.PersonID, Sum(IsNull(L.AmtCr,0) - IsNull(VAdj.AdjustedAmount,0)) AS CNTAmount, Min(L.LedgerId) As LoanLedgerId
	                        FROM Web.People E
	                        LEFT JOIN Web.LedgerAccounts A ON E.PersonID = A.PersonId
	                        LEFT JOIN Web.Ledgers L ON A.LedgerAccountId = L.LedgerAccountId
	                        LEFT JOIN Web.LedgerLines LL ON L.LedgerLineId = LL.LedgerLineId
                            ...
                            AND IsNull(L.AmtCr,0) - IsNull(VAdj.AdjustedAmount,0) > 0
	                        GROUP BY H.ProcessId, E.PersonID
                        ) AS VCNT ON E.PersonID = VCNT.PersonID AND VSalaryLine.ProcessId = VCNT.ProcessId
                        LEFT JOIN (
	                        SELECT H.ProcessId, E.PersonID, Sum(IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0)) AS Advance, Min(L.LedgerId) As LoanLedgerId
	                        FROM Web.People E
	                        LEFT JOIN Web.LedgerAccounts A ON E.PersonID = A.PersonId
	                        LEFT JOIN Web.Ledgers L ON A.LedgerAccountId = L.LedgerAccountId
	                        LEFT JOIN Web.LedgerLines LL ON L.LedgerLineId = LL.LedgerLineId
	                        ...
	                        WHERE H.AdjustmentType = '" + LedgerHeaderAdjustmentTypeConstants.Advance + "'" + @"
                            AND IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0) > 0
	                        GROUP BY E.PersonID , H.ProcessId
                        ) AS VAdvance ON E.PersonID = VAdvance.PersonID AND VSalaryLine.ProcessId = VAdvance.ProcessId
                        WHERE isnull(E.IsActive,1) =1 " +
                        (vm.DepartmentId != null ? " AND E.PersonId IN (SELECT DISTINCT  PP.PersonId " +
                    " FROM web.Processes P " +
                    " LEFT JOIN web.PersonProcesses PP ON PP.ProcessId = P.ProcessId " +
                ...
                else
                {
                    ...
                    mQry = @"SELECT 0 As SalaryHeaderId, E.PersonID AS EmployeeId, P.Name+','+P.Suffix AS EmployeeName, Convert(int,P.Code) AS Code, Convert(Decimal(18,4),DAY(EOMONTH(@DocDate))) - IsNull(VSunday.NoOfSundays,0) AS Days, Isnull(E.BasicSalary,0) as BasicPay, 
                        ...
                        (
                         ...
                         LEFT JOIN 
                        (
	                        SELECT E.EmployeeId, Sum(IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0)) AS DBTAmount, Min(L.LedgerId) As LoanLedgerId
	                        FROM Web.Employees E
	                        LEFT JOIN Web.LedgerAccounts A ON E.PersonID = A.PersonId
	                        LEFT JOIN Web.Ledgers L ON A.LedgerAccountId = L.LedgerAccountId
	                        LEFT JOIN Web.LedgerLines LL ON L.LedgerLineId = LL.LedgerLineId
                        ...
                        (
                            ...
                            AND IsNull(L.AmtCr,0) - IsNull(VAdj.AdjustedAmount,0) > 0
	                        GROUP BY E.EmployeeId
                        ) AS VCNT ON E.EmployeeId = VCNT.EmployeeId
                        WHERE isnull(P.IsActive,1) =1 
                        And E.DateOfJoining <= @DocDate AND E.DateOfRelieving IS NULL " +
            ...
            else
            {
                ...
                {
                    System.Web.HttpContext.Current.Session["SalaryLineReferenceList"] = null;
                    mQry = @"SELECT H.SalaryHeaderId, E.PersonID as EmployeeId, P.Name+','+ P.Suffix AS EmployeeName, Convert(int,P.Code) AS Code, L.Days AS Days, Isnull(E.BasicSalary,0) as BasicPay,  0.00 AS TDS, 0.00 AS TDSBaseValue, 
                        IsNull(VCNT.CNTAmount,0) AS Additions, IsNull(VCNT.CNTAmount,0) AS XAdditions, IsNull(VDBT.DBTAmount,0) AS Deductions, IsNull(VDBT.DBTAmount,0) AS XDeductions,
                        0.00 as RetensionAmount, IsNull(VLoan.LoanEMI,0) AS LoanEMI, IsNull(VAdvance.Advance,0) AS Advance,IsNull(VLoan.LoanEMI,0) AS XLoanEMI, IsNull(VAdvance.Advance,0) AS XAdvance, Convert(nvarchar,H.DocDate) As DocDate, 
                        H.DocTypeId As DocTypeId, H.WagesPayType WagesPayType, H.Remark As HeaderRemark, Convert(Decimal(18,4),DAY(EOMONTH(H.DocDate))) - IsNull(VSunday.NoOfSundays,0) AS MonthDays
                        FROM Web.SalaryHeaders H
                        LEFT JOIN Web.SalaryLines L on H.SalaryHeaderId = L.SalaryHeaderId
        ...
        public IEnumerable<LoanLedgerIdList> GetLoanList(int LedgerAccountId)
        {
            ...
            mQry = @"SELECT L.LedgerId, IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0) As Amount
	                    FROM Web.LedgerHeaders H WITH (Nolock)
	                    LEFT JOIN Web.LedgerLines LL WITH (Nolock) ON LL.LedgerHeaderId = H.LedgerHeaderId
	                    LEFT JOIN Web.Ledgers L WITH (Nolock) ON L.LedgerLineId = LL.LedgerLineId
	                    LEFT JOIN Web.DocumentTypes D WITH (Nolock) ON H.DocTypeId = D.DocumentTypeId
	                    LEFT JOIN 
	                    (
		                    SELECT La.DrLedgerId, Sum(La.Amount) AS AdjustedAmount
		                    FROM Web.LedgerAdjs La WITH (Nolock)
		                    GROUP BY La.DrLedgerId
	                    ) AS VAdj ON L.LedgerId = VAdj.DrLedgerId
	                    WHERE L.LedgerAccountId = @LedgerAccountId
                        AND H.AdjustmentType ='" + LedgerHeaderAdjustmentTypeConstants.Loan + @"'
                        AND IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0) > 0 ";
        ...
        public IEnumerable<AdvanceLedgerIdList> GetAdvanceList(int LedgerAccountId)
        {
            ...
            mQry = @"SELECT L.LedgerId, IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0) As Amount
	                    ...
	                    WHERE L.LedgerAccountId = @LedgerAccountId
                        AND H.AdjustmentType ='" + LedgerHeaderAdjustmentTypeConstants.Advance + @"'
                        AND IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0) > 0 ";
        ...
        public IEnumerable<OtherDeductionLedgerIdList> GetOtherDeductionList(int LedgerAccountId)
        {
            SqlParameter SqlParameterLedgerAccountId = new SqlParameter("@LedgerAccountId", LedgerAccountId);
            ...
            mQry = @"SELECT L.LedgerId, IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0) As Amount
	                    ...
	                    WHERE L.LedgerAccountId = @LedgerAccountId
                        AND H.AdjustmentType ='" + LedgerHeaderAdjustmentTypeConstants.DebitNote + @"'
                        AND IsNull(L.AmtDr,0) - IsNull(VAdj.AdjustedAmount,0) > 0 ";

            IEnumerable<OtherDeductionLedgerIdList> SalaryWizardResultViewModel = db.Database.SqlQuery<OtherDeductionLedgerIdList>(mQry, SqlParameterLedgerAccountId).ToList();

            return SalaryWizardResultViewModel;
        }

        public IEnumerable<OtherAdditionLedgerIdList> GetOtherAdditionList(int LedgerAccountId)
        {
            SqlParameter SqlParameterLedgerAccountId = new SqlParameter("@LedgerAccountId", LedgerAccountId);
    ...
    public class SalaryWizardResultViewModel
    {
        ...
        public Decimal XAdditions { get; set; }
        public Decimal XDeductions { get; set; }
        public Decimal XLoanEMI { get; set; }
    ...
    public class OtherDeductionLedgerIdList
    {
        public int LedgerId { get; set; }
        public Decimal Amount { get; set; }
    }

    public class OtherAdditionLedgerIdList
    {
        public int LedgerId { get; set; }
        public Decimal Amount { get; set; }
    }
