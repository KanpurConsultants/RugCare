    ...
    public class SaleEnquiryLineService : ISaleEnquiryLineService
    {
        ...
        public SaleEnquiryLineViewModel GetSaleEnquiryLineModel(int id)
        {
            ...
            return (from p in db.SaleEnquiryLine
                    ...
                    join SELE in db.SaleEnquiryLineExtended on p.SaleEnquiryLineId equals SELE.SaleEnquiryLineId into tableSELE
                    from tabSELE in tableSELE.DefaultIfEmpty()
                    where p.SaleEnquiryLineId == id
                    select new SaleEnquiryLineViewModel
                    {
                        Amount = p.Amount,
                        ...
                        BuyerSku = tabSELE.BuyerSku,
                        BuyerSpecification = tabSELE.BuyerSpecification,
                        BuyerSpecification1 = tabSELE.BuyerSpecification1,
                        BuyerSpecification2 = tabSELE.BuyerSpecification2,
                        BuyerSpecification3 = tabSELE.BuyerSpecification3,
                    }
        ...
        public IQueryable<SaleEnquiryLineIndexViewModel> GetSaleEnquiryLineListForIndex(int SaleEnquiryHeaderId)
        {
            ...
            var temp = from p in db.SaleEnquiryLine
                       ...
                       join PR in db.Product on p.ProductId equals PR.ProductId into tablePR from tabPR in tablePR.DefaultIfEmpty()
                       orderby p.SaleEnquiryLineId
                       where p.SaleEnquiryHeaderId==SaleEnquiryHeaderId
                       select new SaleEnquiryLineIndexViewModel
                       {
                           BuyerSku= SaleEnquiryLineTab.BuyerSku,
                           DealQty = p.DealQty,
                           DealUnitId=p.DealUnitId,
                           Specification=p.Specification,
                           Rate=p.Rate,
