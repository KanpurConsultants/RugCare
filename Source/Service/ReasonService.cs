...
namespace Service
{
    public interface IReasonService : IDisposable
    {
        ...
        List<ComboBoxResult> GetReasons(int DocumentCategoryId, string term);
    ...
    public class ReasonService : IReasonService
    {
        ...
        public List<ComboBoxResult> GetReasons(int DocumentCategoryId, string term)
        {
            var temp = from p in db.Reason
                       where p.DocumentCategoryId == DocumentCategoryId
                       && (string.IsNullOrEmpty(term) ? 1 == 1 : (p.ReasonName.ToLower().Contains(term.ToLower()) ))
                       orderby p.ReasonName
                       select new ComboBoxResult
                       {
                           text = p.ReasonName,
                           id = p.ReasonId.ToString(),
                       };
