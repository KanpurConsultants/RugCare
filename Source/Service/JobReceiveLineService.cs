...
namespace Service
{
    ...
    public class JobReceiveLineService : IJobReceiveLineService
    {
        ...
        public IEnumerable<JobReceiveLineViewModel> GetLineListForIndex(int headerId)
        {
            ...
            string mQry;
            mQry = @"SELECT L.JobReceiveLineId, isnull(L.Qty,0) + isnull(L.LossQty,0) AS Qty, L.Remark,L.UnitConversionMultiplier,
                    JOH.DocNo AS JobOrderHeaderDocNo, L.Specification, L.JobReceiveHeaderId, L.LossQty, L.LotNo, L.PassQty, L.PenaltyAmt,
                    L.ProductId, P.ProductName, U.UnitName, L.ProductUidId, PU.ProductUidName, L.Qty AS ReceiveQty,
                    L.StockId, L.DealQty,  L.StockProcessId, U.DecimalPlaces UnitDecimalPlaces, L.JobOrderLineId,
                    D1.Dimension1Name,D2.Dimension2Name,D3.Dimension3Name,D4.Dimension4Name,
                    JOH.DocTypeId AS OrderDocTypeId, JOH.JobOrderHeaderId OrderHeaderId, QA.JobReceiveQALineId, QAH.DocTypeId AS JobReceiveQADocTypeId,
                    ...
                    LEFT JOIN web.JobReceiveQALines QA WITH(Nolock)  ON QA.JobReceiveLineId = L.JobReceiveLineId
                    LEFT JOIN web.JobReceiveQAHeaders QAH WITH(Nolock)  ON QAH.JobReceiveQAHeaderId = QA.JobReceiveQAHeaderId
                    ...
                    WHERE L.JobReceiveLineId IS NOT NULL AND H.JobReceiveHeaderId = " + headerId.ToString() + @" ";

            IEnumerable<JobReceiveLineViewModel> pt = db.Database.SqlQuery<JobReceiveLineViewModel>(mQry).ToList();
        ...
        public JobReceiveLineViewModel GetJobReceiveLine(int id)
        {
            return (from L in db.JobReceiveLine
                    ...
                    join JOL in db.JobOrderLine on L.JobOrderLineId equals JOL.JobOrderLineId into JOLTable
                    ...
                    select new JobReceiveLineViewModel
                    {
                        ...
                        UnitDecimalPlaces = JOLtab.Product.Unit.DecimalPlaces,
                        DealUnitDecimalPlaces = JOLtab.DealUnit.DecimalPlaces,
