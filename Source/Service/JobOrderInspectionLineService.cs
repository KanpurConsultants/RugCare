...
namespace Service
{
    public interface IJobOrderInspectionLineService : IDisposable
    {
        ...
        JobOrderInspectionLineViewModel ValidateInspectionBarCode(string ProductUid, int HeaderId);
    ...
    public class JobOrderInspectionLineService : IJobOrderInspectionLineService
    {
        ...
        public JobOrderInspectionLineViewModel ValidateInspectionBarCode(string ProductUid, int HeaderId)
        {
            JobOrderInspectionLineViewModel temp = new JobOrderInspectionLineViewModel();
            ...
            temp = (from p in db.ViewJobOrderBalanceForInspection
                    ...
                    select new JobOrderInspectionLineViewModel
                    {
                        ...
                        JobOrderLineId=p.JobOrderLineId,
                        JobOrderDocNo =jo.DocNo,
                    }).FirstOrDefault();

            if (temp == null)
            {
                temp = new JobOrderInspectionLineViewModel();
        ...
        public JobOrderInspectionLineViewModel GetOrderLineForUid(int Uid)
        {
            ...
            if (SiteId == 1)
            {
                ...
                ProductUid p = db.ProductUid.Where(m => m.ProductUIDId == Uid && m.LastTransactionDocTypeId ==m.GenDocTypeId && m.LastTransactionDocId == m.GenDocId).FirstOrDefault();

                var Rec = (from t in db.JobOrderLine
                           join v in db.ViewJobOrderBalanceForInspection on t.JobOrderLineId equals v.JobOrderLineId
                           join PU in db.ProductUid on v.ProductUidId equals PU.ProductUIDId
                           where v.ProductUidId == p.ProductUIDId && v.BalanceQty > 0
                           select new JobOrderInspectionLineViewModel
