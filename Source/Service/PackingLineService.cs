...
namespace Service
{
    public interface IPackingLineService : IDisposable
    {
        ...
        IEnumerable<ComboBoxResult> GetPendingStockForPacking(int PackingHeaderId, int? ProductId, int? Dimension1Id, int? Dimension2Id, int? Dimension3Id, int? Dimension4Id, string term);
    }
        ...
        public PackingBaleNo FGetNewPackingBaleNo_WithProductId(int PackingHeaderId, int? ProductInvoiceGroupId, int? SaleOrderLineId, int BaleNoPatternId, decimal DealQty, int? ProductId)
        {
            ...
            else
            {
                ...
                if (company.FirstOrDefault().CompanyName == "M/S BHADOHI CARPETS")
                {
                    ...
                    {
                        //MaxBaleNo = BaleNoList.Select(sx => int.TryParse(sx.BaleNo, out x) ? x : 0).Max();
                        LastPackingLineId = BaleNoList.Select(m => m.PackingLineId).Max();
                        PackingLine PL = new PackingLineService(_unitOfWork).Find(LastPackingLineId);

                        MaxBaleNo =  Convert.ToInt16(PL.BaleNo);

                        var LastArea = (from P1 in db.ViewRugSize
                                    where P1.ProductId == PL.ProductId
                                        select new
                                    {
                                         LastArea = P1.StandardSizeArea
                                    }).ToList();
                        ...
                        if ((Decimal)Area.FirstOrDefault().Area <= 6 && (Decimal)LastArea.FirstOrDefault().LastArea <= 6)
                        {
                            MaxBaleNo -= 1;
                        }
        ...
        public IEnumerable<ComboBoxResult> GetPendingStockInForIssue(int PackingHeaderId, int? ProductId, int? Dimension1Id, int? Dimension2Id, int? Dimension3Id, int? Dimension4Id, string term)
        {
            ...
            */
            ...
            string mQry = @"DECLARE @PackingHeaderId INT = " + PackingHeaderId + @" 
                            ...
                            SELECT @filterProductTypes =PS.filterProductTypes, @DocTypeId =PH.DocTypeId, @GodownId = PH.GodownId 
                            FROM web.PackingHeaders PH WITH (Nolock) 
                            LEFT JOIN web.packingsettings PS WITH (Nolock) ON PS.DocTypeId = PH.DocTypeId AND PS.SiteId = PH.SiteId AND PS.DivisionId = PH.DivisionId
                            WHERE PH.PackingHeaderId = @PackingHeaderId 
            ...
            var c = from H in StockInBalanceViewModel
                    select new ComboBoxResult
                    {
                        id = H.StockInId.ToString(),
                        text = H.StockInNo,
                        TextProp1 = "Balance :" + H.BalanceQty,
                        TextProp2 = "Date :" + H.StockInDate,
                        AProp1 = H.ProductName + ((H.ProductUidName == null) ? "" : "," + H.ProductUidName) + ((H.LotNo == null) ? "" : "," + H.LotNo),
                        AProp2 = ((H.StockLotNo == null) ? "" : ",CN:" + H.StockLotNo)
                    };
